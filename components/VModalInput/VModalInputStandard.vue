  ...
  >
    <div class="text-xs text-gray-900 bg-white rounded shadow-lg w-72">
      ...
      <div class="h-64 px-4 overflow-auto">
        <ul ref="options" v-show="filteredOptions.length && !showAdditionalAttributes" class="my-2">
          <li v-for="(option, index) in filteredOptions" :key="option[keyField]">
            <a
            ...
            >
              <span class="truncate" @click.prevent="select(option)">{{ title(option) }}</span>
              <span
...
export default {
  ...
  props: {
    ...
    filterFunction: {
      default: (search, options, nameFields) => {
        return options.filter(option => {
          let title = nameFields.reduce((carry, field) => {
            if (carry === "") {
              return option[field];
            } else {
              return `${carry} ${option[field]}`;
            }
          }, "");
  ...
  computed: {
    filteredOptions() {
      const [options, nameFields, badge] = this.options;
      if (Array.isArray(options)) {
        return this.filterFunction(this.search, options, nameFields);
      } else {
    ...
    badge(option) {
      ...
      if (Array.isArray(options)) {
        const index = options.findIndex(
          item => item[this.keyField] === option[this.keyField]
        );
